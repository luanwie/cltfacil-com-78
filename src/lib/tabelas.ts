 ...
 */
export const calcularINSS = (salario: number, tabela: TabelaINSS): { valor: number; aliquotaEfetiva: number; faixaMarginal: number } => {
...
export const calcularIRRF = (
  ...
  tabela: TabelaIRRF
): { valor: number; aliquotaEfetiva: number; baseCalculoFinal: number; totalDeducoes: number } => {
  if (baseCalculo <= 0) return { valor: 0, aliquotaEfetiva: 0, baseCalculoFinal: 0, totalDeducoes: 0 };
  
  // Deduções
  const deducaoDependentes = Math.min(dependentes, tabela.limiteDependentes) * tabela.deducaoPorDependente;
  const totalDeducoes = deducaoDependentes + pensaoAlimenticia;
  const baseCalculoFinal = Math.max(0, baseCalculo - totalDeducoes);
  
  if (baseCalculoFinal <= 0) return { valor: 0, aliquotaEfetiva: 0, baseCalculoFinal, totalDeducoes };
  
  // Encontra faixa aplicável
  const faixa = tabela.faixas.find(f => baseCalculoFinal <= f.ate) || tabela.faixas[tabela.faixas.length - 1];
...
export const calcularIRRFSync = (baseCalculo: number, dependentes: number, pensao: number, ano = new Date().getFullYear()) => {
